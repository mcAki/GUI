...
public class DateUtil {
	...
	public static String getUpYearByParam(int yearOffest) {
		...
		String nextyear = DateUtil.DateToString(cal.getTime(),
				CM_SHORT_YEAR_FORMAT);
		return nextyear;
	}
	 ...
	 * @return
	 */
	public static boolean isTheDateBetween2Date(Date checkDate, Date alphaDate,
			Date betaDate) {
	 ...
	 * @return
	 */
	public static long subtractTime(Timestamp endTimestamp,
			Timestamp startTimestamp, int flag) {
		int temp = 1000;
		if (flag == 0) {
			temp = 1000;
		} else if (flag == 1) {
	 ...
	 * @return
	 */
	public static Date addMinutes(int mins) {
		Calendar c = Calendar.getInstance();
	...
	/**
	 * 
	 * @Title: addCalendarTimeInMillis
	 * @Description: 根据日历的规则，为给定的日历字段添加或减去指定的时间量。
	 * @param field
	 *            对应相加的时间周期
	 * @param amount
	 *            数量
	 * @return long 当前时间添加或减去后的毫秒数
	 */
	public static long addCalendarTimeInMillis(int field, int amount) {
		Calendar c = Calendar.getInstance();
		c.add(field, amount);
	 ...
	 * 
	 * @Title: timeConvert
	 * @Description: 将给毫秒单位进行转换,如timeConvert(TimeUnit.MINUTES,
	 *               60000)->60000毫秒转换成1分钟
	 * @param targetUnit
	 *            目标单位
	 ...
	 * @return long
	 */
	public static long timeConvert(TimeUnit targetUnit, long sourceDuration) {
		return targetUnit.convert(sourceDuration, TimeUnit.MILLISECONDS);
	}

	@SuppressWarnings("deprecation")
	public static void main(String args[]) {
		long ti = addCalendarTimeInMillis(Calendar.DAY_OF_YEAR, 30);
		System.out.println(timeConvert(TimeUnit.MINUTES, ti
				- System.currentTimeMillis()));
		System.out.println(TimeUnit.HOURS.convert(30, TimeUnit.DAYS));
		// Calendar c = Calendar.getInstance();
		...
		// SimpleDateFormat(DateUtil.CM_LONG_DATE_FORMAT);
		// try {
